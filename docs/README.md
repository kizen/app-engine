...
| [13-setup-assistants.md](13-setup-assistants.md) | The two config wizards (`setup_assistant`, `user_setup_assistant`): every field type and prop, where answers land, how config reaches scripts, re-prompt on hash change, and the view-based form. |
...
| [19-sharing-code-between-scripts.md](19-sharing-code-between-scripts.md) | Importing plain-JS helpers between scripts with relative ESM `import`/`export`: where shared files live, the supported syntax, why shared state is per script, the `imports/*` diagnostics, and how to migrate a plugin with duplicated helpers. |
...
- **Share a helper between scripts** — [19-sharing-code-between-scripts.md](19-sharing-code-between-scripts.md):
  put it in `src/lib/`, `export` it by name, `import { name } from '../../lib/file.js'`.
