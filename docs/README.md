...
| [19-sharing-code-between-scripts.md](19-sharing-code-between-scripts.md) | Importing plain-JS helpers between scripts with relative ESM `import`/`export`: where shared code can live, what's not supported, why shared state is per script, and the build-time errors that catch mistakes. |
...
- **Share a helper between scripts** — [19-sharing-code-between-scripts.md](19-sharing-code-between-scripts.md):
  put it in `src/lib/`, import it with relative ESM `import`.
