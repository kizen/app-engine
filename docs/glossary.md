...
`src/lib/`), holding named exports that scripts import with relative ESM
`import { name } from '../../lib/file.js'`. Also "shared module". The packager folds it into each
importing script at build time — the published script is still one self-contained body, and a
shared file's top-level state is NOT shared: each importer gets its own copy, fresh every run.
Owner: [19-sharing-code-between-scripts.md](19-sharing-code-between-scripts.md).
